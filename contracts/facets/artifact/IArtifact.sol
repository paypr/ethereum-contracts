...
interface IArtifact {
  ...
  /**
   * @notice Returns the number of uses that items start with
   */
  function initialUses() external view returns (uint256);

  /**
   * @notice Returns the number of uses left for the given item
   */
  function usesLeft(uint256 itemId) external view returns (uint256);

  /**
   * @notice Returns the number of uses left for all of the items in circulation
   */
  function totalUsesLeft() external view returns (uint256);

  /**
   * @notice Uses the given item for the given `action`.
   *
   * Emits a {Used} event.
   */
