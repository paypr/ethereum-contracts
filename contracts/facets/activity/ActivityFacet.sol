...
contract ActivityFacet is IActivity {
  function executed(address player) external view override returns (uint256) {
    return ActivityImpl.executed(player);
  }

  function totalExecuted() external view override returns (uint256) {
    return ActivityImpl.totalExecuted();
  }

  function execute(address[] calldata helpers) external override {
    DisableableSupport.checkEnabled();
    ActivityImpl.execute(helpers);
  }
}
