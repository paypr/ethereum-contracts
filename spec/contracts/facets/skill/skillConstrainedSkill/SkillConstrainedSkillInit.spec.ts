...
import { createSkillConstrainedSkill } from '../../../../helpers/facets/SkillConstrainedSkillHelper';
...
describe('initialize', () => {
  it('should set required skills', async () => {
    ...
    const skill = await createSkillConstrainedSkill([
      { skill: skill1.address, level: 1 },
      { skill: skill2.address, level: 2 },
    ]);

    expect<SkillLevel[]>((await asSkillConstrained(skill).requiredSkills()).map(toSkillLevel)).toEqual([
      { skill: skill1.address, level: 1 },
      { skill: skill2.address, level: 2 },
    ]);
  });
